...
ls <- classAssignment(dat, 50, 50)

# Show example rows from output summary
head(ls[[1]])

# Write summary CSV file
write.csv( t( as.data.frame( matrix( unlist( lapply(
  ls[[2]], function(x) {x$class})), 48, 50),
  row.names = as.character( unique( merged$subject)))),
          file = 'confusionsBySub.csv', col.names = F, row.names = F)
